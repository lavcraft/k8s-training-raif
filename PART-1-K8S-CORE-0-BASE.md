...
- Что будет если сервиса на который ссылаемся в ingress не будет?
- Что случится если его добавить во время работы? Почему?
...
- \* Можно ли отправить запрос на ingress, DNS которого не резолвится? Если да то как?
...
- \* Задание: чтобы понять на каких узлах ваше приложение, выведите его в формате имя/узел
...
- В каком случае нельзя без `startupProbe`?
- Как связаны `startupProbe` и `livenessProbe`, `readinessProbe`?
- Можно ли поймать такой же эффект как 500 ошибка при старте, но при остановке приложения?
- Доходят ли запросы через Service если Pod в статусе Terminating?
...
- Объясните поведение
- Как посмотреть текущие ReplicaSet?
- Как понять привязан ли он к Deployment?
...
- как посмотреть какие сейчас есть "балансировщики" в кластере?
...
- [ ] **Then** участники делятся результатами и соображениями
- как вывести текущее значение `username` и `password` секрета `the-secret-of-tasty-sandwich`? Какое оно и почему?
...
- \* Задание: попробуйте найти свой пароль из секрета regcred ( создавался в [задании 02](#hands-on-practice-quest-02-i-have-an-application-i-wish-deploy-it-to-production) )
- \* В чём отличие от секрета? Как закодирован ваш пароль?
...
- [ ] Какую роль играет `topologyKey` для `requiredDuringSchedulingIgnoredDuringExecution`?
- [ ] Можно ли сделать так, чтобы app-knife деплоились только на узлы на которых нет app-butter?
- [ ] Для чего используется anti affinity правила?
...
- [ ] может ли CronJob запустить много Job одновременно? (`kubectl explain cronjob.spec.startingDeadlineSeconds`)
